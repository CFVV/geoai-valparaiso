...
def descargar(cfg: dict, log) -> dict:
    ...
    Retorna un dict con al menos:
        {
            "mosaico_incompleto": bool,
            "ultimo_mes_cubierto": str | None,   # ej. "marzo" — solo si incompleto
            "tiles_descargados": list[int],
            "tiles_fallidos": list[int],
        }
    """
    ...
    s2_out = Path(cfg["rutas"]["s2_mosaics"]) / str(anio_fin)
    ...
    date_start = f"{anio_fin}-01-01"
    date_end = f"{anio_fin + 1}-01-01"

    roi_ee = ee.FeatureCollection(
        [ee.Feature(_tile_to_ee_geom(row)) for _, row in tiles_gdf.iterrows()]
    ).geometry()

    incompleto, ultimo_mes = _detectar_mosaico_incompleto(roi_ee, anio_fin, CLOUD_MAX)
    if incompleto:
    ...
    for i, (_, row) in enumerate(tiles_gdf.iterrows(), 1):
        ...
        if out_tif.exists():
            log.info(f"Descargando tile {i}/{len(tiles_gdf)} (id={tile_id})... ya existe, se salta.")
            descargados.append(tile_id)
            continue

        log.info(f"Descargando tile {i}/{len(tiles_gdf)} (id={tile_id})...")
        try:
            geom_ee = _tile_to_ee_geom(row)
            img, n_imgs = _make_s2_mosaic(geom_ee, date_start, date_end, CLOUD_MAX)
            if n_imgs == 0:
                log.warn(f"Tile {tile_id}: sin escenas S2 en el período {date_start}..{date_end}.")
                fallidos.append(tile_id)
                continue
            ...
            log.error(f"Tile {tile_id}: falló la descarga S2.")
            fallidos.append(tile_id)

    return {
        "mosaico_incompleto": incompleto,
        "ultimo_mes_cubierto": ultimo_mes,
